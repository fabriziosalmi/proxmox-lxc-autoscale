...
### Fixed
- **Shared memory was discounted as reclaimable cache, and the daemon shrank containers into the OOM killer** ([#52](https://github.com/fabriziosalmi/proxmox-lxc-autoscale/pull/52) introduced this): the page-cache exclusion added in 2.0.2 subtracts the `file` key of `memory.stat`, but on cgroup v2 `file` includes `shmem` (tmpfs, `/dev/shm`, shm segments), and shared memory is swap-backed, so on a container with no swap the kernel cannot reclaim it at all. Measured on Proxmox VE 9.1 with a 512 MB container holding 200 MB in `/dev/shm`: the cgroup reported 39.9% in use and the daemon reported **0.41%**, then shrank the container to its 256 MB floor, after which a further 60 MB request produced `oom_kill 1` while the daemon still reported 0.79%. It could not recover, because by its own measurement there was nothing to relieve. Only the genuinely reclaimable part, `file` minus `shmem` (`total_cache` minus `total_shmem` on cgroup v1), is now subtracted. This makes the reported figure differ from the Proxmox UI for shmem-heavy containers, because the UI subtracts the file total whole; the difference is deliberate, since this number decides whether to take memory away. Affects every release from 2.0.2. Workloads with tmpfs are not exotic: `/run` is tmpfs in every systemd container, and PostgreSQL's shared buffers are shared memory.

- **Explicit `cpu_pinning` ranges were written to the container config without being checked** ([#78](https://github.com/fabriziosalmi/proxmox-lxc-autoscale/issues/78)): `_CPU_RANGE_RE` is a syntax check, so on a 32-CPU host `31-0` (reversed), `0-999` (past the end of the host), `99` (a CPU that does not exist) and `0,0,0,0` were all written verbatim into `/etc/pve/lxc/<ctid>.conf`. The kernel refuses a malformed or out-of-range cpuset, LXC then cannot set up the cgroup, and the container does not start, so a typo in one tier's configuration took a container down rather than leaving it unpinned. Ranges are now checked against the online CPU set from `/sys/devices/system/cpu/online` and refused with a message naming what the host has; a reversed range is refused even when the topology probe failed and there is nothing to check membership against, because it is wrong on any host. Values are also stored canonically, so `4,2,0` becomes `0,2,4` and `0,0,0,0` becomes `0`. The pattern now ends in `\Z` rather than `$`, which also matched before a trailing newline.

- **CPU core pinning silently did nothing, on every host of every vendor** ([#75](https://github.com/fabriziosalmi/proxmox-lxc-autoscale/issues/75)): `p-cores` and `e-cores` were resolved from `/sys/devices/system/cpu/cpu*/topology/core_type`, an attribute that does not exist. `drivers/base/topology.c` is the only file that creates `cpuN/topology/`, it installs one attribute group wholesale, and `core_type` is in neither of its two arrays; it is absent at every tag checked from v3.16 to v6.17 and appears in none of the three ABI documents. So the read returned nothing on every machine of every vendor, not only on AMD, and hybrid Intel was affected too: reproduced on an i9-12900 running kernel 7.0.14-11-pve, where `p-cores` wrote `0-23`. With no `core_type` files to read, the detector classified every CPU as a P-core, so `cpu_pinning: p-cores` wrote `lxc.cgroup2.cpuset.cpus: 0-31` into the container config — a pin to every core, which is no pin at all — while `cpu_pinning: e-cores` resolved to nothing and was skipped without a single log line. The detection shell loop also exited non-zero whenever its glob matched no files, so every non-hybrid host logged a spurious `Command failed (rc=1)` on each scaling cycle. Reproduced on an AMD Ryzen AI MAX+ 395 running kernel 7.1.9, where `core_type` is absent and the old resolver returned `0-31` for `p-cores` and `None` for `e-cores`. P and E cores are now read from the perf PMU interface the kernel does create on hybrid parts, `/sys/devices/cpu_core/cpus` and `/sys/devices/cpu_atom/cpus` (`arch/x86/events/intel/core.c`), whose presence is itself the hybrid signal. A tier that asks for a hybrid group on a host that reports none now logs a warning naming the groups that host does have, and does not write a pin. Note that it does not remove one either: a cpuset already present in the container config, including a `0-31` written by an earlier version of this bug, stays until a resolvable `cpu_pinning` value replaces it or you delete the line by hand. Deleting an operator's cpuset because their config has a typo seemed worse than leaving it and saying so.
...
### Added
- **`l3:N` and `numa:N` CPU pinning groups**, available on any CPU vendor. `l3:N` is one L3 cache domain, which on AMD is a CCD/CCX; confining a container to one keeps its working set in a single L3 rather than paying Infinity Fabric latency each time a thread lands on the other side of the package. `numa:N` is one NUMA node addressed by its kernel node id, for multi-socket hosts and for EPYC in NPS2/NPS4. Both are derived from generic sysfs (`cache/index3/shared_cpu_list` and `node*/cpulist`), so they need no vendor-specific support. L3 domains are numbered from the lowest CPU id upward. A NUMA node with no CPUs, such as a CXL or persistent-memory node, is not offered as a group, since an empty cpuset would have been dropped silently by the caller.
- **`all` now means every online CPU**, read from `/sys/devices/system/cpu/online` rather than counted with `nproc`. `nproc` reports the daemon's own affinity mask, so a unit with `CPUAffinity=` or a daemon inside a cpuset saw both the wrong count and the wrong CPU ids, and disagreed with the `l3:N` and `numa:N` groups read from sysfs on the same host.
- **The detected CPU groups are logged once** at first use, including the size of each L3 domain, which is how you tell which CCD carries the extra cache on a Ryzen X3D part. Topology detection is now a single host probe instead of two commands, which matters under `use_remote_proxmox` where each command is an SSH round trip.
