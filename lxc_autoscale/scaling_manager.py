...
async def _delete_scaling_snapshot(base_snapshot: str, snapname: str) -> None:
    """Remove a snapshot this module created, whatever the clone did.

    Only snapshots generated by generate_unique_snapshot_name are passed here,
    so an operator's own snapshots are never touched.
    """
    if not await run_command(["pct", "delsnapshot", str(base_snapshot), snapname],
                             timeout=TIMEOUT_EXTENDED):
        logger.warning(
            "Could not remove scaling snapshot %s on %s; remove it by hand or it "
            "will accumulate", snapname, base_snapshot,
        )
...
async def scale_out(group_name: str, group_config: Dict[str, Any]) -> None:
    ...
    if not await run_command(["pct", "snapshot", str(base_snapshot), unique_snap,
        ...
        # Record the attempt so the grace period throttles the retry. Without
        # this a failing scale-out is retried on every poll, forever.
        scale_last_action[group_name] = _now_tz()
        return

    clone_hostname = generate_cloned_hostname(base_snapshot, len(current_instances) + 1)
    cloned = await run_command(["pct", "clone", str(base_snapshot), str(new_ctid),
                                "--snapname", unique_snap, "--hostname", clone_hostname],
                               timeout=TIMEOUT_EXTENDED)
    # The snapshot exists only to clone from. Leaving it behind accumulates one
    # per attempt on the source container, and a failing scale-out retries every
    # cycle, so this is a disk-exhaustion path on the host rather than clutter.
    await _delete_scaling_snapshot(base_snapshot, unique_snap)
    if not cloned:
        logger.error("Failed to clone %s to %s", base_snapshot, new_ctid)
        scale_last_action[group_name] = _now_tz()
        return
